...
food_features = {
    ...
    "데미소다":  {"ratio":3.5,  "roundness":0.90},  # 수정: 원통형 비율/원형도
...
def extract_features(img):
    ...
    if not cnts: raise ValueError("객체 인식 실패")
...
def classify_snack(r, rd, hue):
    exp_hue = {
      "뻥튀기":0, "데미소다":30,  # 초록매실→데미소다, yellow≈30
      "쫀디기":5, "메가톤":15,
      "월드콘":30, "조리퐁":25,
      "미쯔블랙":120, "앙크림빵":20
    }
...
st.title("📷 푸드스캐너")
st.caption("스낵 사진만 중앙에 올려주세요 → 분류·영양·알레르기·권장량 안내")
